...
import static ru.liga.truckstacker.config.AppGlobalSettings.*;
...
public class Truck {
    ...
    @Column(name = "truck_area", columnDefinition = "TEXT[]")
    ...
    public Truck(char[][] truckArea) {
        ...
        Arrays.stream(truckArea).forEach(t -> Arrays.fill(t, ' '));
    }
    ...
    private boolean isBoxCoordinatesValid(int i, int j) {
        return i >= 0 && j >= 0 && i < truckArea.length && j < truckArea[0].length;
    }
